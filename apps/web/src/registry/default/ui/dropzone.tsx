...
function DropZoneArea({
  ...
  children,
  ...props
}: DropZoneAreaProps & { ref?: React.Ref<HTMLDivElement> }) {
...
function DropzoneDescription(
  props: DropzoneDescriptionProps & { ref?: React.Ref<HTMLParagraphElement> }
) {
...
function DropzoneFileList(
  props: DropZoneFileListProps & { ref?: React.Ref<HTMLOListElement> }
) {
  const { ref, ...rest } = props
  const context = useDropzoneContext()
  if (!context) {
  ...
  return (
    ...
    >
      <li
        ...
        className={cn(
          'flex flex-col justify-center gap-2 rounded-md px-4 py-2',
          className
        )}
      >
        {props.children}
...
function DropzoneFileMessage(
  props: DropzoneFileMessageProps & { ref?: React.Ref<HTMLParagraphElement> }
) {
...
function DropzoneMessage(
  props: DropzoneMessageProps & { ref?: React.Ref<HTMLParagraphElement> }
) {
  const { children, ref, ...rest } = props
  const context = useDropzoneContext()
  if (!context) {
...
function DropzoneRemoveFile(
  props: DropzoneRemoveFileProps & { ref?: React.Ref<HTMLButtonElement> }
) {
  const { className, ref, ...rest } = props
  const context = useDropzoneFileListContext()
  if (!context) {
...
function DropzoneRetryFile(
  props: DropzoneRetryFileProps & { ref?: React.Ref<HTMLButtonElement> }
) {
  const { className, ref, ...rest } = props
  const context = useDropzoneFileListContext()
  if (!context) {
...
function DropzoneTrigger({
  ...
  children,
  ...props
}: DropzoneTriggerProps & { ref?: React.Ref<HTMLLabelElement> }) {
  const { ref, ...rest } = props
  const context = useDropzoneContext()
  if (!context) {
    throw new Error('DropzoneTrigger must be used within a Dropzone')
  }
  const { fileStatuses, getFileMessageId, getInputProps } = context
  ...
  // Get input props from dropzone context
  const inputProps = getInputProps()

  // Create a ref to the input
  const inputRef = useRef<HTMLInputElement>(null)

  // Wrap the onChange to reset input value after selection
  ...
  return (
    <label
      ref={ref}
      className={cn(
        'cursor-pointer rounded-sm bg-secondary px-4 py-2 font-medium ring-offset-background transition-colors focus-within:outline-hidden hover:bg-secondary/80 has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-ring has-[input:focus-visible]:ring-offset-2',
        className
      )}
    >
      {children}
...
function InfiniteProgress({
  className,
  ...props
}: InfiniteProgressProps & { ref?: React.Ref<HTMLDivElement> }) {
  const { ref, ...rest } = props
...
const useDropzone = <TUploadRes, TUploadError = string>(
  ...
  const dropzone = rootUseDropzone({
    ...
    onDropAccepted: async (newFiles) => {
      ...
      const maxFiles = validation?.maxFiles ?? Infinity
      const availableSlots = maxFiles - fileCount
      ...
      if (newFiles.length > availableSlots) {
        if (shiftOnMaxFiles === true) {
          ...
          const filesToRemove = fileStatuses
            .slice(0, newFiles.length - availableSlots)
            .map((f) => f.id)
          await Promise.all(filesToRemove.map((id) => onRemoveFile(id)))
          filesToAdd = newFiles.slice(0, maxFiles)
        } else {
          setRootError(getRootError(['too-many-files'], validation ?? {}))
      ...
      const onDropFilePromises = filesToAdd.map(async (file) => {
