...
import { reactPatternsConfig as config } from '@/config/react-patterns'
...
export default async function DocsLayout({
  ...
  const reactPatternsConfig = await getServerDocsConfig({
      ...
      <div className="container flex-1 items-start md:grid md:grid-cols-[220px_minmax(0,1fr)] md:gap-6 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-10">
        <aside className="sticky top-14 z-30 hidden w-full shrink-0 md:block border-r min-h-[calc(100vh-3.5rem)] self-start">
          <ScrollArea className="h-[calc(100vh-3.5rem)] py-6 pr-6 lg:py-8">
            <DocsSidebarNav
              items={reactPatternsConfig.docs.sidebarNav}
