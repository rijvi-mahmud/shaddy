import { type TypedHooks, allTypedHooks } from 'contentlayer/generated'
...
export async function getDocFromParams({
  params,
}: DocPageProps): Promise<(TypedHooks & { notAvailable: boolean }) | null> {
  let localizedSlug = makeLocalizedSlug(params)
  let doc = allTypedHooks.find((doc) => doc.slugAsParams === localizedSlug)
  ...
  if (!doc) {
    ...
    doc = allTypedHooks.find((doc) => doc.slugAsParams === localizedSlug)
...
export function getBreadcrumb(docSlug: string) {
  const slug = getSlugWithoutLocale(docSlug, 'typed-hooks')
  ...
  const makeBreadcrumb = (
    slug: string,
    config: typeof typedHooksConfig
  ): NavItem[] | null => {
  ...
  const breadcrumbs = makeBreadcrumb(slug, typedHooksConfig)
